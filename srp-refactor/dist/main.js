...
class EmailService {
    ...
    sendBulkEmails(recipients, subject, body) {
...
class TaskNotificationService {
    ...
    createTaskWithNotification(taskName, assigneeEmail) {
        ...
        this.emailService.sendEmail(assigneeEmail, `New Task Assigned: ${taskName}`, `Hello,\n\nYou have been assigned a new task: "${taskName}".\nTask ID: ${taskId}\n\nPlease complete it by the deadline.\n\nBest regards,\nTask Management System`);
        return taskId;
    }
    completeTaskAndNotify(taskId, completerEmail) {
        ...
        this.emailService.sendEmail(completerEmail, `Task Completed`, `Your task (ID: ${taskId}) has been marked as completed. Thank you!`);
    }
}
function demonstrateSRP() {
    ...
    emailService.sendBulkEmails(["alice@example.com", "bob@example.com"], "Meeting Reminder", "Don't forget the meeting at 3 PM");
    console.log("\n3. COORDINATED SERVICE (Uses both):");
    console.log("-----------------------------------");
    const notificationService = new TaskNotificationService();
    notificationService.createTaskWithNotification("Refactor authentication system", "developer@example.com");
    notificationService.createTaskWithNotification("Fix UI bugs", "invalid-email");
}
demonstrateSRP();
